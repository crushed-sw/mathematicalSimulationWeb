<template>
    ...
    >
        ...
        <v-group
            @mousedown="drawLine($event, 'in3')"
            @mouseup="drawLineDone($event, 'in3')"
        >
            <v-path
                :stroke="valueConfig.stroke"
                :strokeWidth="valueConfig.strokeWidth"
...
function out() {
    if(props.in[0] == -1 || props.in[1] == -1 || props.in[2] == -1) {
        return -1;
    }
    return Number(
        (!!props.in[0] && !props.in[1] && !props.in[2]) ||
        (!props.in[0] && !!props.in[1] && !props.in[2]) ||
        (!props.in[0] && !props.in[1] && !!props.in[2]) ||
        (!!props.in[0] && !!props.in[1] && !!props.in[2])
    );
...
const dataValue = "M-45,36.212 A51.212,51.212,45,0,0,-45,-36.212 M-35,36.212 A51.212,51.212,45,0,0,-35,-36.212 M-35,36.212 Q10,40,30,0 M-35,-36.212 Q10,-40,30,0";
const in1Value = "M-50,-20 L-35,-20";
const in2Value = "M-50,0 L-30,0";
const in3Value = "M-50,20 L-35,20";
const outValue = "M50,0 L40,0 A5,5,0,1,0,30,0 A5,5,0,1,0,40,0";
const backValue = "M-35,36.212 A51.212,51.212,45,0,0,-35,-36.212 Q10,-40,30,0 Q10,40,-35,36.212 Z";

function enter(event: any) {
    if(props.isPreview) {return;}
    isPointVisible.value = true;
    store.annotation.value = `in1: ${props.in[0]}<br />in2: ${props.in[1]}<br />in3: ${props.in[2]}<br />out: ${out()}`;
...
function drawLine(event: any, valueStr: string) {
    ...
    const position = store.threeInputGateValues(props.id);
    ...
    } else if(valueStr === "in3") {
        store.drawLine.end = position[2];
        store.drawLine.endId = props.id;
        store.drawLine.endNum = 2;
    } else if(valueStr === "out") {
        store.drawLine.begin = position[3];
...
function drawLineDone(event: any, valueStr: string) {
    ...
    if(valueStr === "in1" && store.drawLine.beginId !== -1) {
        ...
        store.drawLine.endNum = 1;
        store.addLine();
    } else if(valueStr === "in3" && store.drawLine.beginId !== -1) {
        store.drawLine.end = position[2];
        store.drawLine.endId = props.id;
        store.drawLine.endNum = 2;
        store.addLine();
...
watch([() => props.rotate, () => props.scaleX, () => props.scaleY, () => props.x, () => props.y], () => {
    ...
    const in3Line = store.getLine(props.inDependence[2]);
    const outLine = store.getLine(props.outDependence[0]);
    ...
    if(in3Line !== undefined) {
        in3Line.points = [in3Line.points[0], in3Line.points[1], ...position[2]];
    }
    if(outLine !== undefined) {
        outLine.points = [...position[3], outLine.points[2], outLine.points[3]];
    }
}, {deep: true});
</script>
